variable "location"               { default = "northeurope" }
variable "prefix"                 { default = "almalinux8" }
variable "vm_admin"               { default = "azureuser" }
variable "id_rsa_pub"             { default = "ssh-rsa " }
variable "vm_publisher"           { default = "almalinux" }
variable "vm_offer"               { default = "almalinux" }
variable "vm_sku"                 { default = "8_5-gen2" }
variable "vm_version"             { default = "latest" }

# Accept terms
# az vm image list --publisher AlmaLinux  --all --output table
# https://docs.microsoft.com/bs-cyrl-ba/cli/azure/vm/image/terms?view=azure-cli-latest
# az vm image terms accept --urn almalinux:almalinux:8_5-gen2:8.5.20211118